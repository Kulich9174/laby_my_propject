...
export default function Header (){
    ...
    return(
        ...
        <header className='bg-[#1A2633]' >
            ...
            {/* menu bar */}
            <ul id='navID' className={bool ? 'm-0 p-0 w-[65%] sm:absolute sm:left-0 sm:top-[5rem] sm:text-white sm:block sm:w-[110%] sm:bg-[#1A2633] sm:h-calc sm:overflow-auto sm:transition-top delay-[0.4s] desktop:w-[50%] flex justify-between text-white text-xs not-italic font-medium uppercase list-none tracking-[1.2px]':'m-0 p-0 w-[65%] sm:hidden laptop:flex desctop:flex justify-between text-white text-xs not-italic font-medium uppercase list-none tracking-[1.2px] desktop:flex'}>
                <li className='sm:p-2 sm:transition-bg sm:hover:bg-[#28394c] sm:hover:text-inherit hover:text-[#227CE4]'><a href='#'>Главная</a></li>
                <li className='sm:p-2 sm:transition-bg sm:hover:bg-[#28394c] sm:hover:text-inherit hover:text-[#227CE4] laptop:hidden'><a href='#'>О нас</a></li>
                <li className='sm:p-2 sm:transition-bg sm:hover:bg-[#28394c] sm:hover:text-inherit hover:text-[#227CE4]'><a href='#'>Наши склады</a></li>
                <li className='sm:p-2 sm:transition-bg sm:hover:bg-[#28394c] sm:hover:text-inherit hover:text-[#227CE4]'><a href='#'>Брокерам</a></li>
                <li className='sm:p-2 sm:transition-bg sm:hover:bg-[#28394c] sm:hover:text-inherit hover:text-[#227CE4]'><a href='#'>Контакты</a></li>      
            </ul>
            <div className='flex items-center gap-2 sm:pr-5px laptop:float-left'>
                ...
                {/* menu burger */}
                <div id='toggleID' className='pl-[1%] w-32px h-32px sm:pl-2 laptop:hidden desktop:hidden'onClick={()=>{setBool(!bool)}}>
                <svg className={bool ? 'hidden': 'w-max h-max m-auto cursor-pointer transition-opacity delay-[0.4s]'} xmlns="http://www.w3.org/2000/svg" width="26" height="23" viewBox="0 0 26 23" fill="none">
                    <rect width="26" height="1" fill="white"/>
                    <rect x="3" y="11" width="23" height="1" fill="white"/>
                    <rect y="22" width="26" height="1" fill="white"/>
                    </svg>
                    <img src={Close} alt='close' className={bool ? 'w-[32px] h-[32px] m-auto cursor-pointer':'hidden'}/>
                </div>
            </div>
            </div>
        </header>
